...
class Servo_Subscriber():
    ...
    def callback(self, servo_msg):
        if self.manual_checker == 1:        
            ...
            servo_axis_pub = rospy.Publisher('servo%s_axis' % Camera_number, Int8MultiArray, self.callback_manual_axis, queue_size=1)
            servo_axis_msg = Int8MultiArray()
            servo_axis_msg.data = [self.servo_x, self.servo_y]
            servo_axis_pub.publish(servo_axis_msg)
        
        elif self.manual_checker == 0:
            self.manual_subs = rospy.Subscriber('/manual%s_axis' % Camera_number, Int8MultiArray, self.callback_manual_axis, queue_size=1)
...
class Manual_Subscriber():
    def __call__(self): 
        ...
        self.manual_check_subs = rospy.Subscriber('/manual%s_check' % Camera_number, Int8, self.callback_checker, queue_size=1)
        self._Manual_subs = rospy.Subscriber('/manual_control_%s' % Camera_number, Int8MultiArray, self.callback_manual, queue_size=1)
    ...
    def callback_manual(self, manual_msg): 
        if self.manual_checker == 1:
            self.servo_subs = rospy.Subscriber('/servo%s_axis' % Camera_number, Int8MultiArray, self.callback_servo_axis, queue_size=1)       
            ...
            manual_axis_pub = rospy.Publisher('manual%s_axis' % Camera_number, Int8MultiArray, self.callback_servo_axis, queue_size=1)
            manual_axis_msg = Int8MultiArray()
