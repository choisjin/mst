...
class Server():                                                         # Server 연결 및 데이터 전송
    def server_init(self):
        self.db = pymysql.Connect(host='15.164.166.55', user='mstpjt', password='1111', port = 58849, database='mstDB')
        self.cursor = self.db.cursor()

        self.query = "UPDATE mstDB SET CAMERA_NUM = %s, SORTATION = %s, sec = %s WHERE No = 1"
...
class MainWindow(QtWidgets.QMainWindow, Background_Set):                # 기능선택 화면
    ...
    def Start_btn(self):                                # 기능 실행
        self.filePath.clear()
        rospy.init_node('cam_subs', anonymous = False)
...
class Tracking_Finder(QtWidgets.QDialog, Background_Set):               # 객체 인식시 로그 확인 화면
    def __init__(self, camera):
        ...
        self.button_print_speed_up = QtWidgets.QPushButton('Slow_catch', self)
        self.button_print_speed_up.resize(80, 32)
        self.button_print_speed_up.move(5, 5)
        self.button_print_speed_up.clicked.connect(self.print_time_up)

        self.button_print_speed_down = QtWidgets.QPushButton('Fast_catch', self)
        self.button_print_speed_down.resize(80, 32)
        self.button_print_speed_down.move(90, 5)
        self.button_print_speed_down.clicked.connect(self.print_time_down)

        self.tb = QtWidgets.QTextEdit(self)
        self.tb.resize(165, 168)
        self.tb.move(5, 42)
    ...
    def print_time_up(self):
        self.print_time += 5
        if self.print_time > 90:
            self.print_time = 90
        print(self.print_time)
   
    def print_time_down(self):
        self.print_time -= 5
        if self.print_time < 10:
            self.print_time = 10        
        print(self.print_time)
    ...
    def video_convert(self):                            # Video 데이터 Qt 데이터로 변환 및 객체 추적 정보 Publish
        ...
        ############################################################### Yolo Custermize ##########################################################
            ...
            if not bbox_adjusted == False and self.pause == False:
                ...
                if self.face_count == self.print_time and self.stop_start_status == 0 and self.finder_status == 0:
                    print(self.print_time)
...
class Tracking_Camera(QtWidgets.QDialog, Server, Cam_Btn_Set, Background_Set):  # Train & Video 조작 화면
    ...
    def convert2original(self, image, bbox):
        ...
        self.image_h, self.image_w, __ = image.shape

        orig_x       = int(x * self.image_w)
        orig_y       = int(y * self.image_h)
        orig_width   = int(w * self.image_w)
        orig_height  = int(h * self.image_h)
    ...
    def callback(self, data):                           # Cam 데이터 Qt 데이터로 변환 및 객체 인식에 따른 모터 구동 Pub
        ...
        midScreenWindow = 10  # 객체를 인식한 사각형이 중앙에서 벗어날 수 있는 여유 값
        ...
        if not bbox_adjusted == False:
            x = bbox_adjusted[0] -bbox_adjusted[2]/2
            y = bbox_adjusted[1] -bbox_adjusted[3]/2
            w = bbox_adjusted[2]
            h = bbox_adjusted[3]
            self.missing_count = 0
            self.face_count += 1
            ...
            if self.tracking_on_off == 1:
                ...
                self.cam_tracking_pub = rospy.Publisher('cam_tracking_%d' % self.camera, UInt16MultiArray, queue_size=1)
            ...
            if self.face_count == self.print_time and self.finder_status == 0:
        ...
        if self.miss_target == 1 and self.face_count == 0:
            ...
            if not self.people_select == 0:     
                if self.missing_count%30 == 0:
                    ...
                    getmsg = 'People_num : ' + '%d' % self.people_select
                    self.finder.append_text(getmsg)
                    getmsg = 'Missing_time\n' + str((now2 - self.now1)/1000)
                    self.finder.append_text(getmsg)
        ############################################################### server 전송 부 ################################################################
                    # data = (self.camera, self.people_select, str(now2 - self.now1))
                    # self.cursor.execute(self.query, data)
                    # self.db.commit()
        ############################################################### server 전송 부 ################################################################    
            elif self.people_select == 0: 
